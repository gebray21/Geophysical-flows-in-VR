...
namespace Breach.WildWaters.WaterSystems
{
    ...
    public class WaterInteractionBody : MonoBehaviour, IVelocity
    {
        ...
        private void Awake()
        {
            ...
            rigidbody.isKinematic = true; //no movement unless there is landslide
        ...
        private void FixedUpdate()
        {
            ...
            {
                //set drag maximum 
                // isGrounded = Physics.CheckSphere(sphere.transform.position, groundDistance, Ground, QueryTriggerInteraction.Ignore);
                // if (!isGrounded)  //if it  is in air, let it be fall
                //    rigidbody.drag = 0.0f;
                //if (isGrounded && sphere.WaterVelocity.magnitude < 0.01f) //if it is on the ground, but there is no landslide around 
                //  rigidbody.drag = 100.0f;
                if (sphere.WaterVelocity.magnitude > 0.01f) //if no movement 
                {
