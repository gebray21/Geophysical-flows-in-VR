...
namespace Breach.WildWaters.WaterSystems
{
    ...
    public class WaterSphere : MonoBehaviour
    {
        ...
        [SerializeField] private bool isCentric =true;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawWireSphere(transform.position+offSet, radius * transform.lossyScale.magnitude);
        }

        private void FixedUpdate()
        {
            var sphere = isCentric? new Sphere3d(radius * transform.lossyScale.magnitude, transform.position): new Sphere3d(radius * transform.lossyScale.magnitude, transform.position+offSet);
            // Get the water plane
           ...
           // Debug.Log(" velocity : " +  WaterVelocity);
